...
export function openEditThreadModal(thread) {
  const html = `
    <div class="relative bg-white rounded-2xl w-full max-w-md shadow-2xl">
      ...
      <div class="flex items-center justify-between px-5 pt-6 pb-4 border-b border-gray-100">
        <h3 class="text-base font-bold text-gray-900">Edit Thread</h3>
      </div>
      <div class="px-5 py-4 space-y-4">
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1.5">
            Thread name <span class="text-red-500">*</span>
          </label>
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1.5">
            Max members
            <span class="text-gray-400 font-normal">(current: ${thread.member_count ?? 1})</span>
          </label>
...
export function openMeetingNotesRangeModal(onGenerate) {
  const html = `
    <div class="relative bg-white rounded-2xl w-full max-w-xs shadow-2xl">
      ${_closeBtn('thread-notes-range-modal')}
      <div class="px-5 pt-6 pb-4 border-b border-gray-100">
        <h3 class="text-base font-bold text-gray-900">📋 Meeting Notes</h3>
        <p class="text-xs text-gray-500 mt-1">How many recent messages to analyse?</p>
      </div>
  ...
  const html = `
    <div class="relative bg-white rounded-2xl w-full max-w-md shadow-2xl
      ...
      <div class="px-5 pt-6 pb-4 border-b border-gray-100 flex-shrink-0">
        <h3 class="text-base font-bold text-gray-900">📋 Meeting Notes</h3>
        ${messageCount
          ? `<p class="text-xs text-gray-400 mt-0.5">Based on ${messageCount} messages</p>`
          : ''}
      </div>
      ${summary ? `
        <div class="px-5 py-3 bg-emerald-50 border-b border-emerald-100 flex-shrink-0">
          <p class="text-sm text-emerald-800 leading-relaxed">${esc(summary)}</p>
        </div>` : ''}
      <div class="flex-1 overflow-y-auto px-5 py-4">
...
export function openJoinRequestModal(thread, onConfirm) {
  const html = `
    <div class="relative bg-white rounded-2xl w-full max-w-sm shadow-2xl">
      ...
      <div class="px-5 pt-6 pb-4 border-b border-gray-100">
        <h3 class="text-base font-bold text-gray-900 mb-1">Join "${esc(thread.title)}"</h3>
        <p class="text-xs text-gray-400">
          ${thread.member_count} / ${thread.max_members} members
