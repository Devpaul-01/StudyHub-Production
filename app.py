...
class Config:
    ...
    # Fix for Render free tier: stale/dead connections in the pool cause
    # "SSL error: decryption failed or bad record mac" on the first request
    # after a worker restart or idle period.
    # - pool_pre_ping: tests each connection before use; silently replaces dead ones.
    # - pool_recycle: force-recycles connections every 5 min so they never go stale.
    # - pool_size/max_overflow: kept small for Render's free-tier connection limits.
