...
def send_password_reset(email, link):
    """Send password reset email — non-blocking background thread."""
    try:
        html = f"""
        <html>
...
def send_verification_email(email, link):
    ...
    The old version called mail.send() synchronously inside the request cycle.
    On Render's free tier the Gmail SMTP handshake takes long enough to exceed
    Gunicorn's 30-second worker timeout, killing the process before a response
    is ever returned. Sending in a daemon thread releases the worker immediately.
    """
    try:
        html = f"""
        <html>
        ...
        # Grab the real app object before spawning the thread (needed for app context)
        app = current_app._get_current_object()
        thread = Thread(target=send_async_email, args=(app, msg))
        thread.daemon = True
        thread.start()
